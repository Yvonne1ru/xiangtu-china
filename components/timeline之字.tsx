...
// 【优化 1】：缩小卡片内边距和字号，使其更紧凑
function CardContent({ event, onClick }: { event: TimelineEvent; onClick: () => void }) {
  return (
    <div 
      onClick={onClick}
      className="p-3 bg-[#FDFBF7] border border-[#A0522D]/20 rounded-lg cursor-pointer hover:border-[#A0522D] hover:shadow-md transition-all group"
    >
      <h3 className="text-sm font-serif font-bold text-[#333333]/80 mb-1 group-hover:text-[#A0522D] transition-colors">
        {event.title}
      </h3>
      <p className="text-xs text-[#333333]/60 leading-relaxed line-clamp-2">
        {event.event}
      </p>
    </div>
  );
    ...
    <div className="relative w-full h-full bg-[#F5F0E6] flex flex-col overflow-hidden rounded-md">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="relative max-w-5xl mx-auto">
          ...
          {/* 居中的垂直轴线 */}
          <div className="absolute left-1/2 top-0 bottom-0 w-0.5 bg-[#A0522D]/30 -translate-x-1/2" />

          {/* 【优化 2】：将 space-y-12 改为 space-y-4 (16px)，大幅缩小间距 */}
          <div className="space-y-4">
            {events.map((event, idx) => {
              const isRight = idx % 2 === 0;

              return (
                // 【优化 3】：移除 min-h-[100px]，让行高由卡片内容自然撑开
                <div key={idx} className="relative flex items-center">
                  
                  {/* 左侧区域 */}
                  <div className="w-1/2 pr-8 flex justify-end">
                    {!isRight && (
                      <div className="relative pr-6 w-full max-w-sm">
                        <div className="absolute right-0 top-1/2 w-6 h-0.5 bg-[#A0522D]/30 -translate-y-1/2" />
                        <CardContent event={event} onClick={() => setSelectedEvent(event)} />
                      </div>
                    )}
                  </div>

                  {/* 中间时间圆圈 (稍微缩小一点以匹配紧凑布局) */}
                  <button
                    onClick={() => setSelectedEvent(event)}
                    className="absolute left-1/2 -translate-x-1/2 w-10 h-10 rounded-full bg-[#F2EFE9] border-2 border-[#A0522D] flex items-center justify-center transition-all hover:scale-110 hover:bg-[#A0522D] hover:text-[#F5F0E6] shadow-md z-10"
                  >
                  ...
                  <div className="w-1/2 pl-8">
                    {isRight && (
                      <div className="relative pl-6 w-full max-w-sm">
                        <div className="absolute left-0 top-1/2 w-6 h-0.5 bg-[#A0522D]/30 -translate-y-1/2" />
                        <CardContent event={event} onClick={() => setSelectedEvent(event)} />
                      </div>
                    )}
                  </div>
